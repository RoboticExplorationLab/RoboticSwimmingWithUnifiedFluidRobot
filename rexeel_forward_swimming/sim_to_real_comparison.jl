...
# Paths (relative to this environment)
const DATA_DIR = joinpath(@__DIR__, "..", "data")
const VIS_DIR = joinpath(@__DIR__, "..", "visualization")

# Create output directory
output_dir = joinpath(VIS_DIR, "rss_figures")
mkpath(output_dir)
...
for amplitude in amplitude_cases
    ...
    hardware_tracking_path = joinpath(forward_swimming_dir, amplitude, "$(amplitude)_hardware_trajectories.csv")
    simulation_path = joinpath(forward_swimming_dir, amplitude, "$(amplitude)_simulation.jld2")
    motor_angles_path = joinpath(forward_swimming_dir, amplitude, "$(amplitude)_hardware_motor_angles.csv")
